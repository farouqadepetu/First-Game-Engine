...
namespace RenderingEngine
{
	...
	*/
	...
	public:
		...
		* @param[in, optional] isMSAAEnabled Pass in true if you want to have MSAA enabled, false otherwise.
		* @param[in, optional] isTextEnabled Pass in true if you want to have text enabled, false otherwise.
		*/
		RenderScene(unsigned int width, unsigned int height, HWND windowHandle,
			bool isMSAAEnabled = false, bool isTextEnabled = false);
		...
		* @param[in, optional] isTextEnabled Pass in true if you want to have text enabled, false otherwise.
		*/
		void CreateDeviceResources(unsigned int width, unsigned int height, HWND windowHandle,
			bool isMSAAEnabled = false, bool isTextEnabled = false);
		...
		*/
		void CompileShader(unsigned int shaderKey, std::wstring_view filename,
			std::string_view entryPointName, std::string_view target);
		...
		* @param[in] target The name of the shader target to compile with.
		*/
		void CompileShader(std::wstring_view shaderKey, std::wstring_view filename,
			std::string_view entryPointName, std::string_view target);
		...
		*/
		...
		//------------------------------------------------------------------------------------------------------------------------------------
		//INPUT ELEMENT DESCRIPTION FUNCTIONS
		...
		* @param[in, optional] instanceStepRate The number of instances to render. Used for instancing.
		*/
		void CreateInputElementDescription(std::wstring_view key, const char* semanticName, unsigned int semanticIndex,
			DXGI_FORMAT format, unsigned int inputSlot, unsigned int byteOffset,
			D3D12_INPUT_CLASSIFICATION inputSlotClass = D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA,
			unsigned int instanceStepRate = 0);
		...
		* @param[in] offset The offset in descriptors, from the start of the descriptor table.
		*/
		void CreateDescriptorRange(std::wstring_view descriptorRangeKey,
			D3D12_DESCRIPTOR_RANGE_TYPE type, unsigned int numDescriptors, unsigned int shaderRegister, unsigned int registerSpace,
			unsigned int offset);
		...
		* @param[in] shaderRegister The register the sampler is linked to.
		*/
		void CreateStaticSampler(std::wstring_view staticSamplerKey, D3D12_FILTER filter,
			D3D12_TEXTURE_ADDRESS_MODE u, D3D12_TEXTURE_ADDRESS_MODE v, D3D12_TEXTURE_ADDRESS_MODE w, unsigned int shaderRegister);
		...
		//------------------------------------------------------------------------------------------------------------------------------------
		//PIPELINE STATE OBJECT FUNCTIONS
		...
		*/
		...
		/**@brief Links the PSO and its associated root signature to the pipeline to indicate what settings you want to use to render objects.
		...
		*
		*/
		void CreatePSO(std::wstring_view psoKey, D3D12_FILL_MODE fillMode, BOOL enableMultisample,
		...
		*/
		void LinkDynamicBuffer(unsigned int bufferType, unsigned int dynamicBufferKey, unsigned int indexConstantData = 0,
		...
		* The parameters indexConstantData rootParameterIndex are used if the dynamic buffer is a constant buffer.
		*/
		void LinkDynamicBuffer(unsigned int bufferType, std::wstring_view dynamicBufferKey, unsigned int indexConstantData = 0,
			unsigned int rootParameterIndex = 0);
		...
		*/
		void RenderText(const vec4& textLocation, const Color& textColor, float textSize,
	...
	private:
		...
		//Stores all of the shaders for this scene.
		std::unordered_map<std::wstring_view, Microsoft::WRL::ComPtr<ID3DBlob>> mShadersStr;
		...
		//---------------------------------------------------------------------------------------------------------------------
		//INPUT ELEMENT DESCRIPTION HASH MAPS
		...
		//Stores input element descriptions for a set of shaders.
		std::unordered_map<std::wstring_view, std::vector<D3D12_INPUT_ELEMENT_DESC>> mInputElementDescriptionsStr;
		...
		//Stores root parameters for root signatures.
		std::unordered_map<std::wstring_view, std::vector<D3D12_ROOT_PARAMETER>> mRootParametersStr;

		//Stores descriptor ranges for descriptor tables.
		std::unordered_map<std::wstring_view, std::vector<D3D12_DESCRIPTOR_RANGE>> mDescriptorRangesStr;
		...
		//Describes all of the constant data that is expected in a set of shaders.
		//Microsoft::WRL::ComPtr<ID3D12RootSignature> mRootSignature;
		std::unordered_map<std::wstring_view, Microsoft::WRL::ComPtr<ID3D12RootSignature>> mRootSignaturesStr;

		//Stores static samplers.
		std::unordered_map<std::wstring_view, std::vector<D3D12_STATIC_SAMPLER_DESC>> mStaticSamplersStr;
		...
		//---------------------------------------------------------------------------------------------------------------------
		//PIPELINE STATE OBJECT HASH MAPS

		//Stores pipeline state objects.
		std::unordered_map<unsigned int, Microsoft::WRL::ComPtr<ID3D12PipelineState>> mPSOs;

		//Stores pipeline state objects.
		std::unordered_map<std::wstring_view, Microsoft::WRL::ComPtr<ID3D12PipelineState>> mPSOsStr;
		//---------------------------------------------------------------------------------------------------------------------
		...
		//We can't update a dynamic buffer until the GPU
		//is done executing all the commands that reference it, so each frame needs its own dynamic buffer.
		std::unordered_map<std::wstring_view, DynamicBuffer[DeviceResources::NUM_OF_FRAMES]> mDynamicBuffersStr;
